...
if [[ "$link" == "-u" || "$link" == "--update" ]] ; then

    echo "an updated version will be downloaded to this directory"
    wget "https://github.com/meshstyles/little_helpers/releases/download/update_repo/archive_dl.sh" 
    exit 0

elif [[ "$link" == "-f" || "$link" == "--file" ]] ; then

    echo "please use relative path without \"./\""
    listfile="./$2"
